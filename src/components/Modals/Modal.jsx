...
const Modal = ({ open, handleClose, title, description, button, type }) => {
  const icon = () => {
    if (type === "success") {
      return (
      ...
      return (
        <div className="mx-auto flex items-center justify-center h-12 w-12 rounded-full bg-red-100">
          <HiX className="h-6 w-6 text-red-600" aria-hidden="true" />
        </div>
      );
    } else if (type === "alert") {
      return (
        <div className="mx-auto flex items-center justify-center h-12 w-12 rounded-full bg-yellow-100">
          <HiExclamation className="h-6 w-6 text-yellow-600" aria-hidden="true" />
        </div>
      );
    }
  };
